...
FILE = r"D:\machine learning project\Medical Chatbot [END 2 END] [NLP]\models\data1.pth"  # Ensure this path is correct
...
diseases_description = pd.read_csv(r"D:\machine learning project\Medical Chatbot [END 2 END] [NLP]\data\symptom_Description.csv")
diseases_description['Disease'] = diseases_description['Disease'].apply(lambda x: x.lower().strip(" "))

disease_precaution = pd.read_csv(r"D:\machine learning project\Medical Chatbot [END 2 END] [NLP]\data\symptom_precaution.csv")
disease_precaution['Disease'] = disease_precaution['Disease'].apply(lambda x: x.lower().strip(" "))

symptom_severity = pd.read_csv(r"D:\machine learning project\Medical Chatbot [END 2 END] [NLP]\data\Symptom-severity.csv")
symptom_severity = symptom_severity.applymap(lambda s: s.lower().strip(" ").replace(" ", "") if type(s) == str else s)

with open('D:/machine learning project\Medical Chatbot [END 2 END] [NLP]/data/list_of_symptoms.pickle1', 'rb') as data_file:
    symptoms_list = pickle.load(data_file)

with open('D:/machine learning project/Medical Chatbot [END 2 END] [NLP]/models/fitted_model1.pickle', 'rb') as modelFile:
...
def index():
    data = []
    user_symptoms.clear()
    file = open("D:/machine learning project/Medical Chatbot [END 2 END] [NLP]/static/assets/files/ds_symptoms.txt", "r")
